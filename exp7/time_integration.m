...
switch difference_scheme
    ...
    case 'central'
        step_id = 2;
        for lon_id = 2:nlon-1
            u(lon_id,step_id) = u(lon_id,step_id-1) - 0.5*c*dt/dx * ( u(lon_id+1,step_id-1) - u(lon_id-1,step_id-1));
            uk(step_id) = uk(step_id) + u(lon_id,step_id)^2/2;
        end
        ...
        for step_id = 3:ntime
            for lon_id = 2:nlon-1
                u(lon_id,step_id) = u(lon_id,step_id-2) - c*dt/dx * ( u(lon_id+1,step_id-1) - u(lon_id-1,step_id-1));
                uk(step_id) = uk(step_id) + u(lon_id,step_id)^2/2;
            end
